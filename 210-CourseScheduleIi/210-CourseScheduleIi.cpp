// Last updated: 4/5/2026, 11:46:53 AM
class Solution {
public:

   void dfst(int node,vector<int>&vis,vector<vector<int>>&graph,stack<int>&st){
    vis[node]=1;
    for(auto ele: graph[node]){
        if(!vis[ele]) dfst(ele,vis,graph,st);
    }
    st.push(node);
   }
    ...
    vector<int> findOrder(int n, vector<vector<int>>& req) {
        ...
        for(int i=0;i<n;i++){
            if(!vis[i]){
                ...
                pathvis[i]=1;
                if(!dfs(i,graph,vis,pathvis)) return {};
            }
        }
